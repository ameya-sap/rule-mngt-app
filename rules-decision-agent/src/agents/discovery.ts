...
import { LlmAgent, Gemini, BaseLlm, BaseTool, BaseToolset } from '@google/adk';

export interface DiscoveryAgentInput {
  model: BaseLlm;
  tools: (BaseTool | BaseToolset)[];
}

export function createDiscoveryAgent({ model, tools }: DiscoveryAgentInput) {
  return new LlmAgent({
    name: 'DiscoveryAgent',
    model,
    tools,
