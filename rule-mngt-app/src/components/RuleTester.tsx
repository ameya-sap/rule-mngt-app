...
function ResultDisplay({ result, onAddAsExample }: ResultDisplayProps) {
  const formatConditionValue = (value: any) => {
    if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
      const formula = value as { field: string; operator: string; value: number };
      return `[${formula.field}] ${formula.operator} ${formula.value}`;
    }
    return String(value);
  };
             ...
             <div>
                ...
                <div className="space-y-2 text-sm border p-3 rounded-lg bg-background">
                    ...
                    <div key={index} className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="outline">{formatConditionValue(condition.value)}</Badge>
